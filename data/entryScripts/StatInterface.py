...
def getTeam(team, table):
	...
	for p in p_set:
		...
		if table == "game":
			...
			p_stats['GP'] = g.GamesPlayed
			p_stats['MIN'] = g.MinutesPlayed
			p_stats['PPG'] = g.Points
			p_stats['RPG'] = g.Rebounds
			p_stats['APG'] = g.Assists
			p_stats['SPG'] = g.Steals
			p_stats['BPG'] = g.Blocks
			p_stats['TPG'] = g.Turnovers
			p_stats['FG%'] = g.FieldGoalPercentage
			p_stats['FT%'] = g.FreeThrowPercentage
			p_stats['3P%'] = g.ThreePointPercentage
		else:
